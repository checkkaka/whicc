...
- **🖥️ 纯本地流式 ASR** — Nemotron-3.5-ASR (非中文) + Qwen3-ASR (中文) 在 Apple Silicon (MLX) 上运行。**音频不出本机**,不依赖云端转写服务；翻译仍由你配置的 LM Studio / vLLM 提供。
- **🎧 三种音频来源** — 可捕获全部系统音频、麦克风或按 Bundle ID 指定一个应用。目标应用退出时等待,重新启动后自动恢复捕获。
...
- **🪟 稳定实时字幕** — final 稳定字幕保持在上方,持续更新的原文/译文草稿显示在下方；按窗口宽度自然换行,紧凑布局优先保证 final 和实时草稿。
- **🤖 可编辑的自学习术语库** — 可在设置中手动添加/编辑术语,jieba + Hermes 也能继续学习；词库保存在 Application Support 并由翻译进程热加载。
...
4. **选择音频来源** — 使用全部系统音频、麦克风,或选择一个正在运行的应用
5. **开始看视频** — 打开任意外语视频/直播,字幕自动叠加
...
```
系统音频 / 麦克风 / 指定应用 (AudioTee / mic)
        ↓  16kHz PCM chunks
   whicc.py (ASR)                    ← 本地 Apple Silicon MLX
   ├─ Qwen3-ASR-0.6B  (中文)
   └─ Nemotron-3.5-ASR (原生 cache-aware streaming + 质量回退)
...
默认以 Nemotron 启动,经过稳定的语言观察后在 Nemotron 与 Qwen3 之间自动切换。

### 音频来源

设置中提供**全部系统音频**、**麦克风**和**指定应用**。指定应用模式保存 Bundle ID 而不是临时 PID,会跟随应用的 Helper 进程；目标退出后等待同一应用重新启动,不会静默切换音频来源。系统音频和指定应用捕获需要授予 macOS“屏幕与系统音频录制”权限。

### Nemotron 流式延迟

Nemotron 默认使用持久 cache-aware streaming。模型设置提供三档已训练的右上下文：

- `[56,3]` — 320ms,默认低延迟档
- `[56,6]` — 560ms,平衡档
- `[56,13]` — 1.12s,质量档和批处理回退

切换档位会自动重启 ASR 后端。原生流式异常时,当前 final 会使用 `[56,13]` 批处理识别兜底,并保留尚未提交的音频。

---

## ⚙️ 翻译配置

打包版首次启动默认开启翻译管线,但仍需配置 OpenAI 兼容服务后才能产生译文。在 macui 设置面板(齿轮):

1. **服务配置** — 保持外置翻译开启；只需要本地 ASR 草稿时可以关闭
...
- **优先布局**: 历史字幕在顶部,final 在实时草稿上方；紧凑窗口优先隐藏历史
- **稳定草稿**: 新 revision 完成后才替换上一版可读译文,避免短 token 闪烁和倒退
- **自然换行**: final 和实时草稿按当前窗口宽度换行,不通过缩小字体硬塞
- **双语字幕**: 现场切换"原文上 / 译文上"
